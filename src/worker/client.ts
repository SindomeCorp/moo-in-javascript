...
import type { ExecuteRequest, WorkerResponse, WireContext } from './protocol.js';

export interface WorkerTransport {
  postMessage(request: ExecuteRequest): void;
  terminate(): void;
  subscribe(message: (response: WorkerResponse) => void, error: (error: unknown) => void): () => void;
}
export type WorkerFactory = () => WorkerTransport;
export interface WorkerSessionOptions { runtime: Runtime; world: World; workerFactory?: WorkerFactory; parserOptions?: Omit<ParserOptions, 'profile'> }
...
export class WorkerSession {
  ...
  constructor(readonly options: WorkerSessionOptions) {
    ...
    options.runtime.loadWorld(this.#initial, { worldLimits: options.world.limits, limits });
  }
  ...
  load(input: unknown, limits?: SnapshotLimits): void { this.options.runtime.loadWorld(input, { world: this.world, ...(limits ? { limits } : {}) }); }
  ...
  run(source: string, options: WorkerRunOptions = {}): WorkerRun {
    ...
    const snapshot = runtime.saveWorld(world, capacity), before = world.objects(), beforeId = world.nextId;
    ...
    const request: ExecuteRequest = { type: 'execute', id, source, snapshot, worldLimits: world.limits,
    ...
    const cleanup = () => {
      ...
      try { worker?.terminate(); } catch { /* A dead worker is already isolated. */ }
    ...
    try {
      worker = (this.options.workerFactory ?? browserWorkerFactory())();
      unsubscribe = worker.subscribe(message => {
        ...
        try {
          ...
          if (message.type === 'output') {
            ...
            const candidate = runtime.loadWorld(message.snapshot, { worldLimits: world.limits, limits: capacity });
            ...
            if (shouldCommit) {
              ...
              final.changes = candidate.changes(before, beforeId).filter(change => change.kind !== 'object-updated' || !equal(change.before, change.after));
              release(); release = () => {};
              world.replaceWith(candidate);
            }
            cleanup(); resolve(final);
          }
