    ...
    queue = queue.then(async () => {
      ...
      try {
        if (request.type === 'parse') {
          ...
          const runtime = await createRuntime({ ...request.options, hostVerbs });
          try {
            ...
            const world = runtime.loadWorld(request.snapshot, { world: configured, limits: capacity });
            ...
            const result = runtime.run(request.source, { world, context, ...(request.limits ? { limits: request.limits } : {}), runId: request.id,
            ...
            send({ type: 'terminal', id: request.id, result: wire, snapshot: runtime.saveWorld(world, capacity), outputCount: result.output.length });
          } finally { runtime.dispose(); }
        }
      } catch (error) { send({ type: 'host-error', id: request.id, message: error instanceof Error ? error.message : String(error) }); }
    });
    return queue;
  };
}
